...
Vitest is not this project's strategy for async React Server Components. Synchronous Server Components and Client Components can be unit-tested, but async Server Components and complete App Router flows belong in the future Playwright E2E layer. Components must not be converted to Client Components merely to make testing easier.
